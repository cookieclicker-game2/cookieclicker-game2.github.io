...
// Game embedding is now handled statically in HTML with lazy-loading for better performance.
function initLazyLoading() {
  const gameFrame = document.querySelector('#game-frame');
  if (!gameFrame) return;

  const placeholder = gameFrame.querySelector('.game-play-placeholder');
  if (placeholder) {
    const playHandler = () => {
      const src = placeholder.getAttribute('data-src');
      if (src && !gameFrame.querySelector('iframe')) {
        gameFrame.innerHTML = `<iframe src="${src}" width="100%" height="100%" frameborder="0" scrolling="no" allow="autoplay; fullscreen"></iframe>`;
      }
    };
    placeholder.addEventListener('click', playHandler);
  }
}
initLazyLoading();
...
function getSidebarGames() {
 ...
 return GAMES.filter(hasSidebarCategory)
 .sort((a, b) => {
 const da = (a.addedDate || '') + ' ' + (a.publishTime || '00:00:00');
 const db = (b.addedDate || '') + ' ' + (b.publishTime || '00:00:00');
